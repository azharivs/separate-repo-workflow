...
# TODO: No problem as long as we record the commit hash of the merge and start cherry picking AFTER that (tested and works. Use client side git hooks to take care of)

# what if some file f2 is modified via devrepo but is then also modified (or even deleted) via origin?
# both versions of f2 should be merged and rebased onto devrepo upon pulling from origin
